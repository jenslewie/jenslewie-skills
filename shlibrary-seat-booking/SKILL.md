...
- **读取本地认证信息** - 从文件或环境变量读取
- **登录态自动刷新** - profile 文件来源的认证缺失或失效时，会自动拉起登录流程；环境变量来源的认证需要手动更新
...
- 如果认证来自 profile 文件且缺失或失效，脚本会自动拉起登录
- 如果认证来自环境变量且缺失或失效，脚本会直接报错并提示你手动更新
...
- 环境变量：如果 `SHL_LIBRARY_ACCESS_TOKEN / SHL_LIBRARY_SIGN / SHL_LIBRARY_TIMESTAMP` 完整存在，会覆盖文件读取
...
- 如果认证来自环境变量，脚本不会自动刷新；需要你自己更新环境变量
...
1. 按优先级读取环境变量或认证文件
2. 如果认证来自 profile 文件且缺失或失效，自动拉起登录流程；如果认证来自环境变量，则直接报错并提示手动更新
