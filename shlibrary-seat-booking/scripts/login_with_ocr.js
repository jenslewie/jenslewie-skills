#!/usr/bin/env node
/**
 * 上海图书馆登录获取认证信息 (OCR辅助版本)
 *
 * 使用方法:
 *   node login_with_ocr.js [--profile 名称] [--manual]
 *
 * 说明:
 * - 默认会在提供 SHL_LIBRARY_USERNAME / SHL_LIBRARY_PASSWORD 时尝试 OCR 辅助登录。
 * - 如果 OCR 失败或未提供账号密码，脚本会回退为手动登录。
 * - 登录成功后，会复用当前稳定的 queryAuthInfo 链路保存认证信息。
 */
...
const USERNAME_ENV = 'SHL_LIBRARY_USERNAME';
const PASSWORD_ENV = 'SHL_LIBRARY_PASSWORD';

function printUsage() {
  console.log('使用方法:');
  console.log('  node login_with_ocr.js [--profile 名称] [--profile-dir 目录] [--auth-file 文件] [--manual]');
  console.log('');
  console.log('示例:');
...
function parseArgs(args) {
  ...
  return { profileName, profileDir, authFile, manualOnly };
}
...
async function recognizeCaptcha(imagePath) {
  ...
  console.log('🔍 正在使用 OCR 识别验证码...');

  try {
    const result = await Tesseract.recognize(imagePath, 'eng', {
      logger: (message) => {
        if (message.status === 'recognizing text') {
          process.stdout.write(`\r   OCR 进度: ${Math.round(message.progress * 100)}%`);
        }
      }
    });
    ...
    console.log(`   OCR 原始结果: "${String(result.data.text || '').trim()}"`);
    console.log(`   清理后结果: "${text}"`);
    console.log(`   置信度: ${Math.round(result.data.confidence || 0)}%`);

    if (!text || text.length < 4 || (result.data.confidence || 0) < 30) {
      return null;
    }

    return text.slice(0, 6);
  } catch (error) {
    console.log(`   ⚠️ OCR 识别失败: ${error.message}`);
    return null;
  }
}

async function tryFillFirstVisible(page, selectors, value) {
...
async function tryAutoSubmitLogin(page) {
  if (!Tesseract) {
    console.log('ℹ️ 当前环境未安装 tesseract.js，跳过 OCR 自动登录。');
    return false;
  }

  const username = String(process.env[USERNAME_ENV] || '').trim();
  const password = String(process.env[PASSWORD_ENV] || '').trim();
  if (!username || !password) {
    console.log(`ℹ️ 未同时提供 ${USERNAME_ENV} / ${PASSWORD_ENV}，跳过 OCR 自动登录。`);
    return false;
  }

  console.log('🤖 尝试 OCR 辅助登录...');

  const filledUsername = await tryFillFirstVisible(page, [
    'input[name="username"]',
    'input#username',
    'input[placeholder*="用户名"]',
    'input[type="text"]'
  ], username);
  const filledPassword = await tryFillFirstVisible(page, [
    'input[name="password"]',
    'input#password',
    'input[type="password"]'
  ], password);
  ...
  if (!filledUsername || !filledPassword) {
    console.log('ℹ️ 未定位到完整的用户名/密码输入框，回退为手动登录。');
    return false;
  }

  const captchaImage = page.locator('img[src*="captcha"], img[alt*="验证码"], .captcha img').first();
  const captchaInput = page.locator('input[name="captcha"], input#captcha, input[placeholder*="验证码"]').first();
  let captchaCode = null;

  try {
    if (await captchaImage.isVisible({ timeout: 1000 }) && await captchaInput.isVisible({ timeout: 1000 })) {
      const tmpPath = path.join(os.tmpdir(), `shlibrary-captcha-${Date.now()}.png`);
      await captchaImage.screenshot({ path: tmpPath });
      captchaCode = await recognizeCaptcha(tmpPath);
      fs.rmSync(tmpPath, { force: true });
      if (captchaCode) {
        await captchaInput.fill(captchaCode);
        console.log(`✅ OCR 已填入验证码: ${captchaCode}`);
      } else {
        console.log('ℹ️ OCR 未能稳定识别验证码，回退为手动登录。');
        return false;
      }
    } else {
      console.log('ℹ️ 未发现可识别的验证码图片或输入框，回退为手动登录。');
      return false;
    }
  } catch (error) {
    console.log(`ℹ️ OCR 预填失败: ${error.message}，回退为手动登录。`);
    return false;
  }
  ...
  console.log('✅ 已通过表单提交尝试 OCR 登录。');
  return true;
}

async function loginAndGetAuth(outputFile, options) {
  ...
  try {
    ...
    if (!options.manualOnly) {
      autoSubmitted = await tryAutoSubmitLogin(page);
    }

    console.log('');
    if (autoSubmitted) {
      console.log('已尝试 OCR 辅助登录。若页面提示失败，请直接在浏览器里修正并继续。');
    } else {
    ...
    if (!state.capturedAuth) {
      ...
      state.capturedAuth = normalizeAuthPayload(result.payload);
      if (!state.capturedAuth) {
        throw new Error(`queryAuthInfo 返回异常: ${JSON.stringify(result.payload)}`);
      }
    }
...
async function main() {
  try {
    const { manualOnly, ...authContext } = parseArgs(process.argv.slice(2));
    const outputFile = resolveProfileFile(authContext).filePath;

    console.log('========================================');
    console.log('上海图书馆登录获取认证信息 (OCR辅助版本)');
    console.log('========================================');
    console.log(`目标文件: ${outputFile}`);
    console.log('');
    console.log('当前流程:');
    console.log('1. 打开登录页面');
    console.log(`2. 如果同时提供 ${USERNAME_ENV} / ${PASSWORD_ENV}，尝试 OCR 识别验证码并自动登录`);
    console.log('3. OCR 不稳定时回退为手动登录');
    console.log('4. 登录成功后自动打开预约入口并调用 queryAuthInfo');
    console.log('5. 保存 accessToken / sign / timestamp 到 profile 文件');
    console.log('');
    console.log('ℹ️ x-encode 无需保存；book_seat.js 会在每次请求前动态生成');
    console.log('========================================\n');

    const success = await loginAndGetAuth(outputFile, { manualOnly });
